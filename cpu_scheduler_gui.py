...
class SchedulerGUI:
    ...
    def mlfq(self, time_slices, allotments):
        ...
        allotment_used = {p.pid: [0]*4 for p in processes}
        ...
        while any(p.remaining > 0 for p in processes):
            ...
            for level in range(4):
                if queues[level]:
                    ...
                    exec_time = min(ts, p.remaining)
                    start = time
                    time += exec_time
                    p.remaining -= exec_time
                    allotment_used[p.pid][level] += exec_time
                    schedule.append((f"P{p.pid}(Q{level})", start, time))

                    for np in processes:
                        if start < np.arrival <= time and np not in arrived:
                            queues[0].append(np)
                            arrived.append(np)

                    if p.remaining > 0:
                        if allotment_used[p.pid][level] >= allot:
                            if level + 1 < 4:
                                queues[level + 1].append(p)
                            else:
                                queues[level].append(p)
                            allotment_used[p.pid][level] = 0
                        else:
                            queues[level].append(p)
                    else:
                        p.completion = time
