...
void MLFQ() {
    ...
    while (completed < n) {
        ...
        int remainingAllotment = (allotment != 0) ? allotment - processes[idx].allotmentUsed : tq;
        int runTime = (processes[idx].remaining < tq) ? processes[idx].remaining : tq;
        if (allotment != 0 && runTime > remainingAllotment) runTime = remainingAllotment;
        ...
        if (processes[idx].remaining == 0) {
            ...
            if (allotment != 0 && processes[idx].allotmentUsed >= allotment && q + 1 < LEVELS) {
...
int main() {
    ...
    do {
        ...
        do {
            clearScreen();
            printf("CHOOSE ALGORITHM TO RUN:\n[1] FCFS\n[2] SJF\n[3] SRTF\n[4] Round Robin\n[5] MLFQ\n[0] Exit\nChoice: ");
            scanf("%d", &choice);
            clearScreen();
            switch (choice) {
                case 1: FCFS(); break;
